...
import { applyTransformation, transformations, getTransformation } from '@/lib/field-mapper-v4/transformations';
...
export default function MappingPreview({
  ...
  const previewData: PreviewData[] = mappings.map(mapping => {
    ...
    let transformationId = mapping.transformationId;
...
function formatValueForDisplay(value: any): string {
  ...
  if (typeof value === 'object') {
    ...
    if (value.url) {
      return `Imagen: ${value.name || value.url.split('/').pop()}`;
    }
